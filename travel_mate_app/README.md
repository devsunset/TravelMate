...
TravelMate(트래블 메이트) Flutter 모바일 앱. 여행 동반자 검색, 1:1 채팅(채팅 요청 후 신청한/신청받은 채팅방 목록에서 선택해 대화), 커뮤니티 게시판·게시글 상세(작성자에게 채팅 요청), 여행 일정 공유. 사용자 식별은 백엔드 랜덤 ID(users.id)이며 이메일은 수집·저장하지 않습니다. 로그인은 Google만 지원합니다.
